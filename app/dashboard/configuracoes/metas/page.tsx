...
export default function Page() {
  ...
  async function carregar() {
    try {
      ...
      if (!resposta.ok || !dados.ok)
        throw new Error(dados.erro || "Não foi possível carregar metas.");

      const mapa: Record<string, Meta> = {};
      for (const usuario of dados.usuarios || [])
        mapa[usuario.id] = metaPadrao(usuario.id);
      for (const meta of dados.metas || [])
        mapa[meta.usuario_id] = { ...metaPadrao(meta.usuario_id), ...meta };
  ...
  async function salvar(usuarioId: string) {
    ...
    try {
      ...
      if (!resposta.ok || !dados?.ok)
        throw new Error(dados?.erro || "Não foi possível salvar a meta.");
      setMetas((atuais) => ({
        ...atuais,
        [usuarioId]: { ...metaPadrao(usuarioId), ...dados.meta },
      }));
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-600">
                Defina metas por colaborador. A comissão é opcional e pode ficar
                em branco quando a operação não usar pagamento por resgate.
              </p>
            </div>
            <Link
              href="/dashboard"
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <div className="rounded-[26px] border border-blue-100 bg-blue-50 p-5">
            ...
            <p className="mt-1 text-xs font-bold text-blue-700">
              O que não foi feito entra na conta da próxima rotina.
            </p>
          </div>
          <div className="rounded-[26px] border border-emerald-100 bg-emerald-50 p-5">
            ...
            <p className="mt-1 text-xs font-bold text-emerald-700">
              Quando passa da meta, o sistema mostra vantagem.
            </p>
          </div>
          <div className="rounded-[26px] border border-violet-100 bg-violet-50 p-5">
            ...
            <p className="mt-1 text-xs font-bold text-violet-700">
              Cada colaborador pode ter uma meta diferente.
            </p>
          </div>
        </section>
          ...
          <div className="mb-4 flex flex-col justify-between gap-3 md:flex-row md:items-center">
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-slate-500">
                Ajuste metas por colaborador. Comissão é opcional.
              </p>
            </div>
            <input
              value={busca}
                      ...
                      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                        <label className="grid gap-1">
                          ...
                          <input
                            type="number"
                            min={0}
                            value={meta.meta_diaria_agendamentos}
                            onChange={(event) =>
                              alterarMeta(
                                usuario.id,
                                "meta_diaria_agendamentos",
                                parseNumeroCampo(event.target.value, 0),
                              )
                            }
                            className="h-10 w-full max-w-[120px] rounded-xl border border-slate-200 px-3 text-sm font-black outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"
                          />
                        </label>
                        ...
                        <label className="grid gap-1">
                          ...
                          <input
                            type="number"
                            min={0}
                            value={meta.meta_semanal_agendamentos}
                            onChange={(event) =>
                              alterarMeta(
                                usuario.id,
                                "meta_semanal_agendamentos",
                                parseNumeroCampo(event.target.value, 0),
                              )
                            }
                            className="h-10 w-full max-w-[120px] rounded-xl border border-slate-200 px-3 text-sm font-black outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"
                        ...
                        <label className="grid gap-1">
                          ...
                          <input
                            type="number"
                            min={0}
                            value={meta.meta_mensal_agendamentos}
                            onChange={(event) =>
                              alterarMeta(
                                usuario.id,
                                "meta_mensal_agendamentos",
                                parseNumeroCampo(event.target.value, 0),
                              )
                            }
                            className="h-10 w-full max-w-[120px] rounded-xl border border-slate-200 px-3 text-sm font-black outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"
                        ...
                        <label className="grid gap-1">
                          ...
                          <input
                            type="number"
                            min={0}
                            value={meta.comissao_por_venda ?? ""}
                            placeholder="Opcional"
                            onChange={(event) =>
                              alterarMeta(
                                usuario.id,
                                "comissao_por_venda",
                                parseComissao(event.target.value),
                              )
                            }
                            className="h-10 w-full max-w-[150px] rounded-xl border border-slate-200 px-3 text-sm font-black outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"
                          />
                          <span className="text-[10px] font-bold text-slate-400">
                            {dinheiro(meta.comissao_por_venda)}
                          </span>
                        </label>
                      </div>
